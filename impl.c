...
digit* add_dig(digit* head, unsigned int val){
    digit* new_dig = (digit*) malloc(sizeof(digit));
    new_dig->val = val;
    new_dig->next = head;
    return new_dig;
}
...
void add_num(digit* a_num, digit* b_num, unsigned int carry){
    ...
    if(b_num == NULL && carry ==0)
        return ;
    if(a_num->next == NULL){
        if(b_num!=NULL && b_num->next != NULL){
            a_num->next = b_num->next;
            b_num->next = NULL;
        }
        else if(b_num == NULL && carry != 0 && (a_num->val+carry) >= BASE){
            digit* new_dig = (digit*) malloc(sizeof(digit));
            new_dig->val = (a_num->val+carry)/BASE;
            new_dig->next = NULL;
            a_num->next = new_dig;
            a_num->val = (a_num->val+carry) % BASE;
            return ;
        }
        else if (b_num!=NULL && b_num->next == NULL && (a_num->val+b_num->val + carry) >= BASE){
            digit* new_dig = (digit*) malloc(sizeof(digit));
            new_dig->val = (a_num->val+b_num->val + carry)/BASE;
            new_dig->next = NULL;
            a_num->next = new_dig;
            a_num->val = (a_num->val+b_num->val + carry) % BASE;
            return ;
        }
    }
    val = a_num->val;
